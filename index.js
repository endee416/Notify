...
if (beforeStatus === 'cart' && afterStatus === 'pending') {
  ...
  vsnap.forEach(doc => {
    ...
    if (Expo.isExpoPushToken(u.pushToken)) {
      messages.push(makeMessage(u.pushToken, `Hi ${u.firstname||'there'}! New pending order: ${data.foodItem}`));
    }
  });
}
...
if (afterStatus === 'dispatched') {
  ...
  csnap.forEach(doc => {
    ...
    if (Expo.isExpoPushToken(c.pushToken)) {
      messages.push(makeMessage(c.pushToken, `Hi ${c.firstname||'there'}! Your order (${data.foodItem}) is now dispatched.`));
    }
  });
}
...
if (beforeStatus === 'dispatched' && afterStatus === 'completed') {
  ...
  vsnap2.forEach(doc => {
    ...
    if (Expo.isExpoPushToken(v.pushToken)) {
      messages.push(makeMessage(v.pushToken, `Hi ${v.firstname||'there'}! Order ${data.foodItem} completed. Your account has been credited.`));
    }
  });
  // driver
  if (data.driveruid) {
    ...
    dsnap2.forEach(doc => {
      ...
      if (Expo.isExpoPushToken(dv.pushToken)) {
        messages.push(makeMessage(dv.pushToken, `Hi ${dv.firstname||'there'}! Your account has been credited.`));
      }
    });
  }
}
...
// Schedule daily at 10:17 AM Nigeria (9:17 UTC) cron.schedule('15 8 * * *', () => { console.log('Triggering daily cron at', new Date().toISOString()); sendDailyNotifications(); });
